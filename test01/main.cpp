...
public:
	ViewerWidget(QWidget* parent = 0, Qt::WindowFlags f = 0, 
		osgViewer::ViewerBase::ThreadingModel threadingModel = osgViewer::CompositeViewer::SingleThreaded) : QWidget(parent, f)
	{
	...
	QWidget* addViewWidget(osgQOpenGLWindow* gw, osg::Node* scene)
	{
	...
	osgQOpenGLWindow* createGraphicsWindow(int x, int y, int w, int h, const std::string& name = "", bool windowDecoration = false)
	{
		...
		return new osgQOpenGLWindow(NULL);
	}
